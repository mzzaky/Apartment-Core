...
public enum ShopItem {
    ...
    VAULT_EXPANSION("Vault Expansion", Material.ENDER_CHEST, ShopBuffType.INCOME_CAPACITY,
            new double[] { 2.5, 5.0, 7.5, 10.0, 12.5 }, // capacity increase % per tier
            new double[] { 6000, 14000, 24000, 38000, 60000 }); // cost per tier
    ...
    public double getBuffValue(int tier) {
        if (tier < 1 || tier > 5)
            return 0.0;
        try {
            com.aithor.apartmentcore.ApartmentCore plugin = com.aithor.apartmentcore.ApartmentCore
                    .getPlugin(com.aithor.apartmentcore.ApartmentCore.class);
            if (plugin != null && plugin.getConfigManager() != null) {
                org.bukkit.configuration.file.FileConfiguration shopConfig = plugin.getConfigManager().getShopConfig();
                String path = "items." + this.name().toLowerCase() + ".buff_values.tier_" + tier;
                if (shopConfig.contains(path)) {
                    return shopConfig.getDouble(path);
                }
            }
    ...
    public double getTierCost(int tier) {
        ...
        try {
            ...
            if (plugin != null && plugin.getConfigManager() != null) {
                org.bukkit.configuration.file.FileConfiguration shopConfig = plugin.getConfigManager().getShopConfig();
                String path = "items." + this.name().toLowerCase() + ".costs.tier_" + tier;
                if (shopConfig.contains(path)) {
                    return shopConfig.getDouble(path);
                }
            }
    ...
    public String getDescription() {
        return switch (this) {
            ...
            case VAULT_EXPANSION -> "Increases your apartment's maximum income capacity";
        };
    }
    ...
    public String getBuffDescription(int tier) {
        ...
        return switch (buffType) {
            ...
            case INCOME_CAPACITY -> String.format("+%.1f%% max income capacity", value);
        };
    }
}