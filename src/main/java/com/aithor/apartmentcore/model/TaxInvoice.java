...
public class TaxInvoice {
    ...
    public long daysSinceCreated(long now) {
        return ageMillis(now) / 86_400_000L;
    }

    public long daysOverdue(long now) {
        if (now < dueAt) return 0;
        return (now - dueAt) / 86_400_000L;
    }
    ...
    public static TaxInvoice deserialize(Map<String, Object> data) {
        ...
        inv.dueAt = ((Number) data.getOrDefault("dueAt", inv.createdAt + 3 * 86_400_000L)).longValue();
    ...
    // Helper for reading from a ConfigurationSection (safety with Bukkit's typed maps)
