...
public class TaxManagementGUI extends PaginatedGUI {
    ...
    protected List<GUIItem> loadItems() {
        List<Apartment> apartments = plugin.getApartmentManager().getApartments().values().stream()
                ...
                .sorted(Comparator.comparing((Apartment a) -> a.computeTaxStatus(System.currentTimeMillis()).ordinal()).reversed()) // Most urgent first
    ...
    private void addActionButtons() {
        ...
        // Pay All button
        Material payAllMaterial = totalUnpaid > 0 ? (canAfford ? Material.EMERALD_BLOCK : Material.RED_CONCRETE) : Material.GRAY_CONCRETE;
        ItemStack payAllItem = new ItemBuilder(payAllMaterial)
                ...
                .lore(
                    ...
                    "",
                    totalUnpaid > 0 ? (canAfford ? "&a▶ Click to pay all" : "&cInsufficient funds") : "&7No taxes due"
                )
                .glow()
                .build();
        ...
        ItemStack taxInfoItem = new ItemBuilder(Material.BOOK)
                .name("&6📊 Tax Overview")
                .lore(
                    "&7Summary of your tax situation",
                    "",
                    "&e📋 Statistics:",
                    "&7• Total Apartments: &f" + apartmentCount,
                    "&7• Overdue Apartments: &c" + overdueCount,
                    "&7• Active Invoices: &f" + totalInvoices,
                    "&7• Total Amount Due: &c" + plugin.getConfigManager().formatMoney(totalUnpaid),
                    "",
                    "&7Taxes are calculated as 2.5% per level",
                    "&7of your apartment's purchase price",
                    "&7and are billed every 24 hours."
                )
                .build();
    ...
    private void handlePaySingleInvoice(Apartment apartment, TaxInvoice invoice) {
        ...
        if (!plugin.getEconomy().has(player, invoice.amount)) {
            GUIUtils.sendMessage(player, "&cInsufficient funds! Need: " + plugin.getConfigManager().formatMoney(invoice.amount));
            GUIUtils.playSound(player, GUIUtils.ERROR_SOUND);
            return;
        }
