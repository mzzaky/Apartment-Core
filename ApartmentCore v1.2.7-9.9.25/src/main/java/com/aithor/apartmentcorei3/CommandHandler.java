...
public class CommandHandler implements TabCompleter {
    ...
    public boolean handleCommand(CommandSender sender, String[] args) {
        ...
        switch (subCommand) {
            case "version":
                sender.sendMessage(ChatColor.GREEN + "ApartmentCore version 1.2.6");
                return true;
            ...
            case "tax":
                ...
                switch (taxSub) {
                    case "info":
                        return handleTaxInfo((Player) sender);
                    case "pay":
                        return handleTaxPay((Player) sender);
                    case "auto":
                        if (args.length != 3) {
                            sender.sendMessage(ChatColor.RED + "Usage: /apartmentcore tax auto <on|off>");
                            return true;
                        }
                        return handleTaxAuto((Player) sender, args[2]);
                    default:
                        sender.sendMessage(ChatColor.RED + "Usage: /apartmentcore tax <info|pay|auto> [on/off]");
                        return true;
                }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.YELLOW + "/apartmentcore tax <info|pay|auto> [on/off]" + ChatColor.WHITE + " - Manage tax system (global per account)");
        ...
        try {
            switch (property.toLowerCase()) {
                ...
                case "tax":
                    ...
                    sender.sendMessage(ChatColor.GREEN + "Set tax for " + apt.displayName + " to " + configManager.formatMoney(apt.tax));
                    break;
                case "tax_days":
                    apt.taxDays = Integer.parseInt(value);
                    sender.sendMessage(ChatColor.GREEN + "Set tax days for " + apt.displayName + " to " + apt.taxDays);
                    break;
                case "rate":
                    ...
                    return true;
                default:
                    sender.sendMessage(ChatColor.RED + "Unknown property. Use: owner, price, tax, tax_days, level, rate");
                    return true;
            }
        } catch (NumberFormatException e) {
