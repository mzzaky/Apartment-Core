...
public class ApartmentPlaceholder extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, String params) {
        ...
        // Auction placeholders: %apartmentcore_auction_<id>_<property>%
        if (parts[0].equalsIgnoreCase("auction") && auctionManager != null) {
            if (parts.length < 3) return "Invalid Auction Placeholder";
            String aptId = parts[1];
            String property = String.join("_", Arrays.copyOfRange(parts, 2, parts.length));
            
            ApartmentAuction auction = auctionManager.getAuction(aptId);
            if (auction == null) return "No Auction";

            return switch (property) {
                case "current_bid" -> configManager.formatMoney(auction.currentBid);
                case "starting_bid" -> configManager.formatMoney(auction.startingBid);
                case "current_bidder" -> auction.currentBidderName.isEmpty() ? "None" : auction.currentBidderName;
                case "owner" -> auction.ownerName;
                case "total_bids" -> String.valueOf(auction.totalBids);
                case "time_remaining" -> formatTime(auction.getRemainingTime());
                case "is_active" -> auction.isActive() ? "Yes" : "No";
                case "has_ended" -> auction.hasEnded() ? "Yes" : "No";
