...
public class MyApartmentsGUI extends PaginatedGUI {
    ...
    private GUIItem createApartmentItem(Apartment apartment) {
        ...
        lore.add("&a▶ Right-click for quick actions");
    ...
    protected void handleItemClick(GUIItem item, InventoryClickEvent event) {
        ...
        if (clickType == ClickType.SHIFT_LEFT || clickType == ClickType.SHIFT_RIGHT) {
            ...
            // Quick actions menu (could be implemented later)
            handleQuickActions(apartment);
        } else {
    ...
    private void handleQuickActions(Apartment apartment) {
        // For now, just show a message about quick actions
        // This could open a sub-menu with common actions
        GUIUtils.sendMessage(player, "&eQuick actions for &f" + apartment.displayName + "&e:");
        GUIUtils.sendMessage(player, "&7• &f/apartmentcore rent claim " + apartment.id);
        GUIUtils.sendMessage(player, "&7• &f/apartmentcore upgrade " + apartment.id);
        GUIUtils.sendMessage(player, "&7• &f/apartmentcore teleport " + apartment.id);
        player.closeInventory();
    }
