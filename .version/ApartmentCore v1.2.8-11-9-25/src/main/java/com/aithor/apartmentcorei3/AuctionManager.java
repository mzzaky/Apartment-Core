...
public class AuctionManager {
    ...
    public void loadAuctions() {
        ConfigurationSection section = dataManager.getAuctionConfig().getConfigurationSection("auctions");
    ...
    public void saveAuctions() {
        if (dataManager.getAuctionConfig() == null) return;

        dataManager.getAuctionConfig().set("auctions", null);
        ...
        for (Map.Entry<String, ApartmentAuction> entry : activeAuctions.entrySet()) {
            ...
            dataManager.getAuctionConfig().set("auctions." + apartmentId, auction.serialize());
        }

        dataManager.saveAuctionFile();
    ...
    public boolean createAuction(Player player, String apartmentId, double startingBid, int durationHours) {
        ...
        if (auctionFee > 0 && !economy.has(player, auctionFee)) {
        ...
        // Charge auction fee
        if (auctionFee > 0) {
            economy.withdrawPlayer(player, auctionFee);
        }
    ...
    public boolean placeBid(Player player, String apartmentId, double bidAmount) {
        ...
        double requiredBid = auction.currentBid + minIncrement;
        if (bidAmount < requiredBid) {
            player.sendMessage(ChatColor.RED + "Bid must be at least " + 
                configManager.formatMoney(requiredBid));
            return false;
        }
        ...
        if (auction.currentBidderId != null) {
            ...
            economy.depositPlayer(previousBidder, auction.currentBid);
            
            if (previousBidder.isOnline()) {
                previousBidder.getPlayer().sendMessage(ChatColor.YELLOW + 
                    "You have been outbid on " + apartmentManager.getApartment(apartmentId).displayName + 
        ...
        long extendThreshold = configManager.getAuctionExtendThreshold() * 60 * 1000L; // minutes to ms
        if (remainingTime < extendThreshold) {
            long extension = configManager.getAuctionExtendTime() * 60 * 1000L; // minutes to ms
            auction.endTime = Math.min(auction.endTime + extension, 
                auction.startTime + configManager.getAuctionMaxDuration() * 60 * 60 * 1000L);
        ...
        if (auction.currentBidderId != null) {
            ...
            if (currentBidder.isOnline()) {
                currentBidder.getPlayer().sendMessage(ChatColor.YELLOW + 
                    "Auction for " + apartmentManager.getApartment(apartmentId).displayName + 
    ...
    private void processAuctionEnd(String apartmentId, ApartmentAuction auction) {
        ...
        if (auction.currentBidderId != null) {
            ...
            economy.depositPlayer(owner, sellerAmount);
        ...
        return activeAuctions.values().stream()
            .filter(auction -> {
                switch (filter.toLowerCase()) {
                    ...
                    case "ending":
                        return auction.getRemainingTime() < 3600000; // Less than 1 hour
                    case "nobids":
            ...
            .sorted((a1, a2) -> Long.compare(a1.getRemainingTime(), a2.getRemainingTime()))
